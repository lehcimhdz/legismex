...
class DofDocumento(BaseModel):
    ...
    """
    seccion: str = Field(description="Sección de la gaceta donde se presentó (ej. UNICA SECCION)")
    organismo: str = Field(description="Poder u Organismo Autónomo que publica (ej. PODER EJECUTIVO)")
    dependencia: str = Field(description="Secretaría o Entidad específica (ej. SECRETARIA DE GOBERNACION)")
...
class DofEdicion(BaseModel):
    ...
    documentos: List[DofDocumento] = Field(default_factory=list, description="Listado de todos los documentos y decretos publicados")
