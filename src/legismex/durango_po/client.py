...
class DurangoPoClient:
    ...
    def _parsear_ediciones_lista(self, html: str) -> List[DurangoPoEdicion]:
        ...
        for link in links:
            ...
            if parent:
                ...
                titulo = h3.get_text(separator=" ", strip=True) if h3 else f"Edición {uuid[:8]}"
                fecha = fecha_span[0].get_text(strip=True) if len(fecha_span) > 0 else "N/A"
                pubs = fecha_span[1].get_text(strip=True) if len(fecha_span) > 1 else ""
                ...
                if card:
                    ...
                    titulo = h3.get_text(separator=" ", strip=True) if h3 else f"Edición {uuid[:8]}"
                    fecha = time_tag.get_text(strip=True) if time_tag else "N/A"
                    es_bis = "Bis" in titulo
                    es_extraordinario = "Ext" in titulo or "Extra" in titulo
    ...
    def _extraer_pdf(self, detail_html: str) -> str:
        ...
        links = re.findall(r'https://transp23\.s3\.amazonaws\.com/[^\"]*\.pdf', detail_html)
