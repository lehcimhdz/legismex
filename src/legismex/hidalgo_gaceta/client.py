...
class HidalgoGacetaClient:
    ...
    def _parsear_listado(self, html: str) -> List[HidalgoGaceta]:
        ...
        for item in items:
            ...
            tipo = tipo_span.get_text(
                strip=True) if tipo_span else "DESCONOCIDO"

            # Título
            titulo_h3 = item.find("h3")
            titulo = titulo_h3.get_text(
                strip=True) if titulo_h3 else "Sin título"
    ...
    def _parsear_detalle(self, html: str, session_id: str) -> HidalgoGacetaDetalle:
        ...
        titulo = header.get_text(
            strip=True) if header else "Sesión " + session_id
        ...
        for card in cards:
            title_div = card.find("h3") or card.find(
                "div", class_="text-slate-900")
            if not title_div:
                continue
            ...
            url_upper = url.upper()
            es_existente = not any(
                p in url_upper for p in PATRONES_INEXISTENTES)
