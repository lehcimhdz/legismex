...
def test_consejeria():
    ...
    for g in gacetas:
        print(f"- Fecha: {g.fecha} | Numero: {g.numero} | Tiene PDF: {g.tiene_pdf}")
    
    if gacetas:
        target = next((g for g in gacetas if g.tiene_pdf), None)
        if target:
            print(f"\\nIniciando descarga de {target.numero}...")
            ruta = cliente.descargar_gaceta(target, criterio="1811", ruta_destino="/tmp/gaceta_consejeria.pdf")
            if ruta and os.path.exists(ruta):
                print(f"Archivo guardado exitosamente: {ruta} ({os.path.getsize(ruta)} bytes)")
            else:
