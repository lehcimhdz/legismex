...
def analyze_historico():
    ...
    with httpx.Client(timeout=30.0, verify=False) as client:
        ...
        for i, form in enumerate(forms):
            ...
            for inp in inputs:
                print(f"  Input: {inp.get('name')} | Type: {inp.get('type') or inp.name} | Value: {inp.get('value')}")
...
def test_documentos_anio(anio, page=1):
    ...
    with httpx.Client(timeout=30.0, verify=False) as client:
        ...
        print(f"Documentos encontrados en la pagina {page}: {len(docs_parsed)}")
        ...
        if pagination:
            ...
            for p in page_items:
                a_tag = p.find("a")
                if a_tag:
                    print(f"  Href: {a_tag.get('href')} | Text: {a_tag.get_text(strip=True)}")
                else:
                    print(f"  Text: {p.get_text(strip=True)} (Active/Disabled)")
