...
def run_sync():
    ...
    ediciones = client.obtener_ediciones(ChiapasAdministracion.ADMIN_2024_2030, anio=2024, mes=12)
    
    print(f"Total de ediciones síncronas encontradas: {len(ediciones)}")
    for ed in ediciones[:3]:
        print(f"Num {ed.numero} | Fecha {ed.fecha} | Sec {ed.seccion} | Par {ed.parte}")
